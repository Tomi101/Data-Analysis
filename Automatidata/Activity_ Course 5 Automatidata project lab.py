...
# 1.  **Handling Outliers**
# 
# Outliers and extreme data points can have a substantial impact on linear regression equations. After visualizing the data, develop a strategy to manage outliers. This may involve removing specific rows, replacing extreme values with the mean, or excluding data points that fall beyond 3 standard deviations from the mean.
# 
# 2.  **Dealing with Missing Data**
# 
# Exploratory Data Analysis (EDA) also involves identifying and addressing missing data. This process helps analysts decide whether to exclude or retain missing values by filling them in using methods like mean, median, or other imputation techniques.
# 
# 3.  **Addressing Multicollinearity and Variable Distributions**
# 
# It's essential to check for multicollinearity among predictor variables and understand their distributions. Doing so allows you to determine which statistical inferences can be drawn from the model and which cannot.
# 
# 4.  **Feature Engineering**
# 
# Additionally, it can be useful to engineer new features by multiplying variables together or taking the difference from one variable to another. For example, in this dataset you can create a `duration` variable by subtracting `tpep_dropoff` from `tpep_pickup time`.
...
# 
# 2. Use the [`map()`](https://pandas.pydata.org/docs/reference/api/pandas.Series.map.html#pandas-series-map) method on the `mean_distance` series. 
# Pass `grouped_dict` as its argument. Reassign the result back to the `mean_distance` series.
...
# ### Check perferomance on full dataset
